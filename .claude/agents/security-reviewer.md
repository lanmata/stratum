...
description: Read-only OWASP Top-10 audit agent for the Angular 20 + Express 5 BFF stack — session-token handling, secrets detection, injection vectors in proxy paths, and dependency CVEs. Invoke as a subagent before releases or after auth changes.
---
...
See `.claude/skills/security-reviewer.skill.md` for the full OWASP checklist and grep commands.
See `.claude/skills/auth-flow.skill.md` for token storage and guard patterns.
