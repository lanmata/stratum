...
@Component({
  selector: 'app-notice-types-list',
  standalone: true,
  ...
  template: `
    <div class="min-h-screen bg-gray-50 p-6 dark:bg-gray-900">
      <div class="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 class="mt-1 text-xl font-semibold text-gray-900 dark:text-gray-100">Tipos de Aviso</h1>
        </div>
        <a
          routerLink="/notice-types/new"
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
              @for (nt of noticeTypes(); track nt.id) {
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  <td class="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">{{ nt.name }}</td>
                  <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ nt.description ?? '—' }}</td>
                  <td class="px-4 py-3">
                    @if (nt.active) {
                  ...
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-1">
                      <a
                        [routerLink]="['/notice-types', nt.id, 'edit']"
                        title="Editar"
    ...
    @if (showConfirm()) {
      <app-confirm-dialog
        message="¿Eliminar este tipo de aviso? Esta acción no se puede deshacer."
...
export class NoticeTypesListComponent implements OnInit {
  private readonly service = inject(NoticeTypeService);
  ...
  protected onDeleteClick(nt: NoticeType): void {
    this.pendingId.set(nt.id);
  ...
  protected onConfirmed(): void {
    ...
    this.service.delete(id).subscribe({
      next: () => {
        this.toast.success('Tipo de aviso eliminado');
        this.load();
      },
      error: () => this.toast.error('Error al eliminar el tipo de aviso'),
    });
  }
  ...
  private load(): void {
    ...
    this.service.getAll().subscribe({
      next: (data) => {
        this.noticeTypes.set(data);
        this.loading.set(false);
      },
      error: () => {
        this.toast.error('Error al cargar los tipos de aviso');
        this.loading.set(false);
      },
    });
  }
