...
import { NoticeTypeRequest } from '@shared/models/notice-type.model';

@Component({
  selector: 'app-notice-type-form',
  standalone: true,
...
export class NoticeTypeFormComponent implements OnInit {
  ...
  private noticeTypeId: string | null = null;
  ...
  ngOnInit(): void {
    this.noticeTypeId = this.route.snapshot.paramMap.get('noticeTypeId');
    if (this.noticeTypeId) {
      this.isEditMode.set(true);
      this.loadForEdit(this.noticeTypeId);
    }
  }
  ...
  protected submit(): void {
    ...
    const call = this.isEditMode()
      ? this.service.update(this.noticeTypeId!, req)
      : this.service.create(req);

    call.subscribe({
      next: () => {
        this.toast.success(
          this.isEditMode() ? 'Tipo de aviso actualizado' : 'Tipo de aviso creado'
        );
        this.router.navigate(['/notice-types']);
      },
      error: () => {
        this.toast.error('Error al guardar el tipo de aviso');
  ...
  private loadForEdit(id: string): void {
    ...
    this.service.getById(id).subscribe({
      ...
      error: () => {
        this.toast.error('Error al cargar el tipo de aviso');
        this.router.navigate(['/notice-types']);
      },
    });
  }
