---
name: API Contract
description: Shared — API path constants, request/response TypeScript interfaces, and BFF-to-Angular path alignment (Developer, API Designer)
...
## `API` Object Structure (from `api.constants.ts`)
...
- Static paths: string literals (e.g., `ROOT: '/v1/users'`)
...
// Response from create — suffix CreateResponse
...
getById(id: string): Observable<UserTO> {
  return this.http.get<UserTO>(API.USERS.BY_ID(id));  // uses constant
}

create(req: UserCreateRequest): Observable<UserCreateResponse> {
  return this.http.post<UserCreateResponse>(API.USERS.ROOT, req);  // uses constant
}
...
4. Create `server/routes/<entity>.routes.js` (see `bff-proxy.skill.md`)
