...
description: OWASP Top-10 audit procedure, grep commands, and threat model for the Angular 20 + Express 5 BFF stack in front-backbone-rest
...
| backbone-rest proxy | All entity data | Unauthorized access if token not forwarded, mass assignment via unfiltered body |
...
# (run npm audit — see npm-audit.tool.md)
```
...
- [ ] A07 — `logout()` in `AuthService` calls `store.clear()` which removes token via NgRx effect
- [ ] A09 — All proxy errors logged at `error` level with URL and message (no PII)
...
- Run `npm audit` findings through `validate_cves` before reporting
